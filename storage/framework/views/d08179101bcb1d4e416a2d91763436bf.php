<?php $__env->startSection('title', 'Merchant Directory - CMarket'); ?>
<?php $__env->startSection('page-title', 'Global Merchant Network'); ?>
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 lg:p-12 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row justify-between items-center gap-8 md:gap-10 overflow-hidden relative group">
        <div class="relative z-10 w-full lg:w-auto text-center lg:text-left">
            ...
            <p class="text-slate-400 dark:text-slate-500 font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] ml-1">Protocol Partners • <?php echo e($merchants->total()); ?> Identified Entities</p>
        </div>
        <div class="grid grid-cols-2 lg:flex items-center gap-3 md:gap-4 relative z-10 w-full lg:w-auto">
            <div class="px-6 py-4 md:px-8 md:py-6 rounded-2xl md:rounded-3xl bg-slate-950 text-white text-center flex-1 lg:w-32 shadow-xl shadow-slate-900/20">
                ...
                <div class="text-base md:text-xl font-black"><?php echo e($merchants->where('status', 'active')->count()); ?></div>
            </div>
            <div class="px-6 py-4 md:px-8 md:py-6 rounded-2xl md:rounded-3xl bg-amber-500 text-white text-center flex-1 lg:w-32 shadow-xl shadow-amber-500/20">
                ...
                <div class="text-base md:text-xl font-black"><?php echo e($merchants->where('status', 'pending')->count()); ?></div>
            </div>
        </div>
        <!-- Decor -->
                        ...
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="px-10 py-8">
                                <div class="flex items-center gap-6">
                                    ...
                                    <div>
                                        <div class="text-sm font-black text-slate-800 dark:text-white mb-1"><?php echo e($merchant->business_name); ?></div>
                                        <div class="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest"><?php echo e($merchant->business_type); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-8">
                                <div class="text-[10px] font-black text-slate-800 dark:text-white uppercase tracking-widest mb-1"><?php echo e($merchant->user->name); ?></div>
                                <div class="text-[10px] font-bold text-sky-500"><?php echo e($merchant->phone); ?></div>
                            </td>
                            <td class="px-10 py-8 text-center">
                                <span class="inline-flex px-4 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-[9px] font-black text-slate-800 dark:text-white tracking-tighter">
                                    <?php echo e($merchant->products_count ?? 0); ?> DEP. UNITS
                                </span>
                            </td>
                            <td class="px-10 py-8 text-center">
                            ...
                            <td class="px-10 py-8 text-right">
                                <div class="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0 duration-300">
                                    <a href="<?php echo e(route('admin.merchants.show', $merchant)); ?>" class="px-6 py-3 rounded-xl bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest hover:bg-sky-500 transition-all shadow-xl shadow-slate-900/10">
                                        Inspect Node ⚡
                                    </a>
                                </div>
                            </td>
