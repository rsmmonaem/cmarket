<?php $__env->startSection('title', 'KYC Audit Terminal - CMarket'); ?>
<?php $__env->startSection('page-title', 'KYC Verification Pipeline'); ?>
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-slate-900 dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 lg:p-12 text-white shadow-2xl shadow-slate-900/10 flex flex-col lg:flex-row justify-between items-center gap-8 md:gap-10 overflow-hidden relative group">
        <div class="relative z-10 w-full lg:w-auto text-center lg:text-left">
            ...
            <p class="text-slate-400 font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] ml-1">Live Audit Queue • <?php echo e($kycs->total()); ?> Identified Nodes</p>
        </div>
        <div class="grid grid-cols-2 lg:flex items-center gap-3 md:gap-4 relative z-10 w-full lg:w-auto">
            <div class="p-4 md:p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl text-center flex-1 lg:w-32">
                ...
                <div class="text-base md:text-xl font-black text-white"><?php echo e($kycs->where('status', 'pending')->count()); ?></div>
            </div>
            <div class="p-4 md:p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl text-center flex-1 lg:w-32">
                <div class="text-[7px] md:text-[8px] font-black text-emerald-500 uppercase mb-1">Passed</div>
                <div class="text-base md:text-xl font-black text-white"><?php echo e($kycs->where('status', 'approved')->count()); ?></div>
            </div>
        </div>
        <!-- Decor -->
                        ...
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="px-10 py-8">
                                <div class="flex items-center gap-6">
                                    <div class="w-12 h-12 rounded-2xl bg-slate-900 border border-slate-100 flex items-center justify-center text-white font-black text-sm shadow-lg group-hover:scale-110 transition-transform">
                                        <?php echo e(substr($kyc->user->name, 0, 1)); ?>

                                    </div>
                                    <div>
                                        <div class="text-sm font-black text-slate-800 dark:text-white mb-1"><?php echo e($kyc->user->name); ?></div>
                                        <div class="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest"><?php echo e($kyc->user->phone); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-8">
                                <div class="text-[10px] font-black text-slate-800 dark:text-white uppercase tracking-widest mb-1"><?php echo e($kyc->id_type); ?></div>
                                <div class="text-[10px] font-bold text-sky-500"><?php echo e($kyc->id_number); ?></div>
                            </td>
                            <td class="px-10 py-8">
                                <div class="text-[10px] font-black text-slate-800 dark:text-white mb-1"><?php echo e($kyc->created_at->format('d M Y')); ?></div>
                                <div class="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-tight"><?php echo e($kyc->created_at->diffForHumans()); ?></div>
                            </td>
                            ...
                            <td class="px-10 py-8 text-right">
                                <div class="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0 duration-300">
                                    <a href="<?php echo e(route('admin.kyc.show', $kyc)); ?>" class="px-6 py-3 rounded-xl bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest hover:bg-sky-500 transition-all shadow-xl shadow-slate-900/10">
                                        Review Detail ⚡
                                    </a>
                                </div>
                            </td>
