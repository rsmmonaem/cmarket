<?php $__env->startSection('title', 'Financial Controller - CMarket'); ?>
<?php $__env->startSection('page-title', 'Global Wallet Infrastructure'); ?>
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 lg:p-12 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row justify-between items-center gap-8 md:gap-10 overflow-hidden relative group">
        <div class="relative z-10 w-full lg:w-auto text-center lg:text-left">
            ...
            <p class="text-slate-400 dark:text-slate-500 font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] ml-1">Managing <?php echo e($wallets->total()); ?> Financial Nodes</p>
        </div>
        
        <div class="w-full lg:w-auto relative z-10">
            <form method="GET" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1 relative">
                    <input type="text" name="search" placeholder="Search identity..." value="<?php echo e(request('search')); ?>"
                ...
                <select name="wallet_type" class="h-14 md:h-16 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-6 text-xs font-black text-slate-800 dark:text-white focus:ring-2 focus:ring-sky-500/20 transition-all">
                    ...
                    <option value="main" <?php echo e(request('wallet_type') == 'main' ? 'selected' : ''); ?>>Main</option>
                    <option value="cashback" <?php echo e(request('wallet_type') == 'cashback' ? 'selected' : ''); ?>>Cashback</option>
                    <option value="commission" <?php echo e(request('wallet_type') == 'commission' ? 'selected' : ''); ?>>Commission</option>
                    <option value="shop" <?php echo e(request('wallet_type') == 'shop' ? 'selected' : ''); ?>>Shop</option>
                </select>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $wallets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wallet): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="px-10 py-8">
                                <div class="flex items-center gap-6">
                                    <div class="w-12 h-12 rounded-2xl bg-slate-900 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-white font-black text-sm shadow-lg group-hover:scale-110 transition-transform">
                                        <?php echo e(substr($wallet->user->name, 0, 1)); ?>

                                    </div>
                                    <div>
                                        <div class="text-sm font-black text-slate-800 dark:text-white mb-1"><?php echo e($wallet->user->name); ?></div>
                                        <div class="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest"><?php echo e($wallet->user->phone); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-8">
                            ...
                            <td class="px-10 py-8 text-center">
                                <div class="text-lg font-black text-slate-900 dark:text-white leading-none">৳<?php echo e(number_format($wallet->balance, 2)); ?></div>
                                <div class="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase mt-1 tracking-tighter">Updated <?php echo e($wallet->updated_at->diffForHumans()); ?></div>
                            </td>
                            <td class="px-10 py-8 text-center">
                            ...
                            <td class="px-10 py-8 text-right">
                                <div class="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0 duration-300">
                                    <a href="<?php echo e(route('admin.wallets.show', $wallet)); ?>" class="w-12 h-12 rounded-2xl bg-slate-900 text-white flex items-center justify-center text-xl hover:bg-sky-500 transition-all shadow-xl shadow-slate-900/20">
                                        ⚖️
                                    </a>
                                </div>
                            </td>
