...
@section('title', 'Admin Command - CMarket')
@section('page-title', 'Global Overview')
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Users Logic -->
        <x-admin.card class="relative overflow-hidden group border-none bg-slate-900 text-white shadow-2xl shadow-slate-900/10">
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-8">
                    <span class="text-[9px] font-black uppercase tracking-[0.2em] opacity-40">User Base Hub</span>
                    <span class="text-xl">👥</span>
                </div>
                <h3 class="text-4xl font-black mb-2 tracking-tighter">{{ number_format(\App\Models\User::count()) }}</h3>
                <p class="text-[9px] font-black text-emerald-400 uppercase tracking-widest">+{{ \App\Models\User::where('created_at', '>=', now()->subDays(7))->count() }} growth last 7d</p>
            </div>
            <!-- Background element -->
        ...
        <x-admin.card class="relative overflow-hidden group border-none bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all duration-500">
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-8">
                    <span class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">KYC Auth Pipeline</span>
                    <span class="text-xl">🛡️</span>
                </div>
                <h3 class="text-4xl font-black text-slate-800 dark:text-white mb-2 tracking-tighter">{{ number_format(\App\Models\Kyc::where('status', 'pending')->count()) }}</h3>
        ...
        <x-admin.card class="relative overflow-hidden group border-none bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all duration-500">
            <div class="relative z-10">
                ...
                <h3 class="text-4xl font-black text-slate-800 dark:text-white mb-2 tracking-tighter">{{ number_format(\App\Models\Order::count()) }}</h3>
        ...
        <x-admin.card class="relative overflow-hidden group border-none bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all duration-500">
            <div class="relative z-10">
                ...
                <h3 class="text-4xl font-black text-slate-800 dark:text-white mb-2 tracking-tighter">{{ number_format(\App\Models\Withdrawal::where('status', 'pending')->count()) }}</h3>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="px-6 md:px-10 py-6 md:py-8 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-50/30 dark:bg-slate-800/30">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="text-[9px] font-black text-sky-500 uppercase tracking-widest hover:text-slate-900 dark:hover:text-white transition-colors">Global Log →</a>
                </div>

                <div class="divide-y divide-slate-50 dark:divide-slate-800">
                    @php $recentOrders = \App\Models\Order::with('user')->latest()->take(7)->get(); @endphp
                    @forelse($recentOrders as $order)
                        <div class="p-6 md:p-8 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group">
                            <div class="flex items-center gap-4 md:gap-6">
                                ...
                                <div>
                                    <h4 class="text-xs md:text-sm font-black text-slate-800 dark:text-white">#{{ $order->order_number }}</h4>
                                    <p class="text-[8px] md:text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{{ $order->user->name ?? 'Protocol Guest' }}</p>
                                </div>
                            </div>
                            <div class="text-right flex items-center gap-4 md:gap-8">
                                <div class="hidden sm:block">
                                    ...
                                    <p class="text-xs md:text-sm font-black text-slate-900 dark:text-white">৳{{ number_format($order->total_amount, 0) }}</p>
                                </div>
                                <div class="w-24 md:w-32 text-right">
                                    <span class="px-2 md:px-3 py-1 rounded-lg text-[7px] md:text-[8px] font-black uppercase tracking-wider
                                        @if($order->status === 'paid' || $order->status === 'delivered') bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400
                                        @elseif($order->status === 'pending') bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400
                                        @else bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 @endif">
                                        {{ $order->status }}
                                    </span>
                                </div>
                            </div>
        ...
        <div class="space-y-8">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <a href="{{ route('admin.kyc.index') }}" class="p-6 rounded-3xl bg-slate-900 dark:bg-slate-800 text-white hover:bg-sky-600 dark:hover:bg-sky-600 transition-all shadow-xl shadow-slate-900/10 flex flex-col items-center text-center gap-4">
                        <span class="text-3xl">🛡️</span>
                        <span class="text-[9px] font-black uppercase tracking-widest">KYC Audit</span>
                    </a>
                    <a href="{{ route('admin.users.index') }}" class="p-6 rounded-3xl bg-indigo-600 text-white hover:bg-sky-600 transition-all shadow-xl shadow-indigo-600/10 flex flex-col items-center text-center gap-4">
                        <span class="text-3xl">👥</span>
                        <span class="text-[9px] font-black uppercase tracking-widest">User Base</span>
                    </a>
                    <a href="{{ route('admin.products.index') }}" class="p-6 rounded-3xl border-2 border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex flex-col items-center text-center gap-4 group/btn">
                    ...
                    <a href="{{ route('admin.wallets.index') }}" class="p-6 rounded-3xl border-2 border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex flex-col items-center text-center gap-4 group/btn">
                        <span class="text-3xl grayscale group-hover/btn:grayscale-0 transition-all">💰</span>
            ...
            <div class="bg-gradient-to-br from-rose-500 to-indigo-600 rounded-[2.5rem] p-10 text-white relative overflow-hidden group shadow-2xl shadow-indigo-500/20">
                <div class="relative z-10">
                    ...
                    <a href="{{ route('admin.settings.index') }}" class="inline-flex items-center gap-3 px-6 py-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/20 transition-all">
                        Edit System Control ⚡
                    </a>
                </div>
                <!-- Decor -->
