...
@section('title', 'Reconfigure Asset - EcomMatrix')
...
<div class="max-w-4xl mx-auto space-y-12 animate-fade-in">
    ...
    <div class="card-premium bg-[#0f172a] p-10 md:p-14 text-white border-none shadow-2xl relative overflow-hidden group">
        <div class="relative z-10 lg:w-2/3">
            ...
            <p class="text-slate-400 font-medium leading-relaxed text-sm md:text-base">Updating visual node #{{ $banner->id }} in the platform matrix. Define its updated priority and destination link.</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="title" value="{{ old('title', $banner->title) }}" placeholder="Enter promotional title" 
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="position" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl text-[11px] font-black text-slate-700 dark:text-white uppercase tracking-widest focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all shadow-sm">
                            ...
                            <option value="popup_banner" {{ $banner->position == 'popup_banner' ? 'selected' : '' }}>Popup Banner</option>
                            <option value="footer_banner" {{ $banner->position == 'footer_banner' ? 'selected' : '' }}>Footer Banner</option>
                            <option value="main_section_banner" {{ $banner->position == 'main_section_banner' ? 'selected' : '' }}>Main Section Banner</option>
                        </select>
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <div x-data="{ photoPreview: '{{ $banner->image ? asset('storage/' . $banner->image) : null }}' }" class="relative flex flex-col items-center">
                            <input type="file" name="image" class="hidden" x-ref="photo" 
                                   ...
                                   ">
                            <div class="w-full h-40 bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[2rem] flex flex-col items-center justify-center text-center group cursor-pointer hover:border-primary transition-all overflow-hidden relative"
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col justify-end">
                            <label class="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-4 px-6 rounded-2xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-all border border-slate-100 dark:border-slate-700">
                                ...
                                <div x-data="{ enabled: {{ $banner->is_active ? 'true' : 'false' }} }">
