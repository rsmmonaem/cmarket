...
@section('title', 'Generation Analytics - CMarket')
@section('page-title', 'My Generations')
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm overflow-hidden relative">
        <div class="flex items-center gap-5 mb-10">
            ...
            <div>
                <h3 class="text-xl font-black text-slate-800 uppercase tracking-tight">Generation Hierarchy</h3>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Real-time tracking of your 9-level partner network</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            ...
            <div class="p-6 rounded-3xl {{ $data['count'] > 0 ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20' : 'bg-slate-50 text-slate-400' }} border border-transparent transition-all duration-300 hover:scale-[1.05]">
                ...
                <p class="text-[9px] font-black uppercase mt-2 tracking-tighter">Partners</p>
            </div>
            @endforeach
        </div>
    </div>
                ...
                <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $data['count'] }} Partners Found</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50">
                                ...
                                <th class="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Access Point</th>
                                <th class="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Career Rank</th>
                                <th class="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Acquisition Date</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="p-6">
                                    <span class="px-3 py-1 rounded bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase tracking-wider">
                                        {{ $member->currentDesignation?->designation->name ?? 'BASE PARTNER' }}
                                    </span>
                                </td>
